...
def evaluate(args, category, model, visualizer, global_step=0):
    ...
    with torch.no_grad():
        ...
        de_st_IAPS = IAPS().to(device)
        de_st_AUPRO = AUPRO().to(device)
        de_st_AUROC = AUROC().to(device)
        de_st_AP = AveragePrecision().to(device)
        de_st_detect_AUROC = AUROC().to(device)
        seg_IAPS = IAPS().to(device)
        seg_AUPRO = AUPRO().to(device)
        seg_AUROC = AUROC().to(device)
        seg_AP = AveragePrecision().to(device)
        seg_detect_AUROC = AUROC().to(device)
        ...
        return round(float(auc_de_st), 4), round(float(ap_de_st), 4), round(float(aupro_de_st), 4), round(
            float(auc_detect_de_st), 4), round(float(iap_de_st), 4), round(float(iap90_de_st), 4), \
            round(float(auc_seg), 4), round(float(ap_seg), 4), round(float(aupro_seg), 4), round(float(auc_detect_seg),
                                                                                                 4), round(
            float(iap_seg), 4), round(float(iap90_seg), 4)
...
def test(args, category):
    ...
    run_name = f"S2TKD_MVTec_test_{category}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_path", type=str, default="../best_pth1/")
    parser.add_argument("--base_model_name", type=str, default="S2TKD_MVTec_11000_")
