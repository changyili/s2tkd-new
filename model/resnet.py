...
class MultiFeatureFusion(nn.Module):
    def __init__(self, channel = 64):
        ...
        #     nn.Conv2d(channel * 4 * 2, channel * 4 * 2, kernel_size=3, stride = 2, padding = 1),
        #     nn.BatchNorm2d(channel * 4 * 2),
        #     nn.ReLU(),
        # )
        self.conv_ = nn.Sequential(
            nn.Conv2d(channel * 7, channel * 7, kernel_size=3, stride = 2, padding = 1),
            nn.BatchNorm2d(channel * 7),
            nn.ReLU(),
        )
...
class BN_layer(nn.Module):
    def __init__(self,
        ...
        # self.conv5 = conv3x3(512 * block.expansion, 512 * block.expansion, 2)
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes*7 //4, planes * block.expansion, stride),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes*7//4, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        #l1 = F.interpolate(x[1], size=x[0].shape[-1], mode='bilinear', align_corners=True)
        # l0 = self.relu(self.bn1(self.conv1(x[0])))
        # l2 = F.interpolate(x[2], size=x[1].shape[-1], mode='bilinear', align_corners=True)
        #print('l0',l0.shape)
        #print('l2', l2.shape)
        #print('x[0]',x[0].shape)
        #print('x[1]',x[1].shape)
        ...
        # feature = torch.cat([l0, x[1], l2],1)
        # print('feature1', feature.shape)
        # feature = self.relu(self.bn5(self.conv5(feature)))
        # print('feature2', feature.shape)
        # print('feature2', x[2].shape)
        feature = self.fusion(x[0], x[1], x[2])
